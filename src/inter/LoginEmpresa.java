...
public class LoginEmpresa extends JPanel {
	...
	public LoginEmpresa() {
		...
		btn_Próximo = new JButton("Pr\u00F3ximo");
		
		btn_Próximo.setBounds(125, 328, 208, 27);
		panel.add(btn_Próximo);
		btn_Próximo.setForeground(new Color(0, 0, 0));
		btn_Próximo.setFont(new Font("Arial", Font.BOLD, 16));
		btn_Próximo.setBackground(new Color(219, 112, 147));
		...
		JLabel lblNewLabel_2 = new JLabel("Fa\u00E7a o login na sua empresa");
		...
		lb_login.setIcon(new ImageIcon(LoginEmpresa.class.getResource("/inter/LOGIN.png")));
	...
	public boolean logarEmpresa(IEmpresaFachada fachada) throws SenhasDiferentesException, JaExisteException, NaoExisteException, StringVaziaException, LoginInvalidoException
	{
		if(this.text_user.getText().length() > 0 && this.pass.getPassword().length > 0)
		{
			return fachada.entrarEmpresa(this.text_user.getText(), this.pass.getPassword());
		}else if(this.text_user.getText().length() <= 0)
		{
			throw new StringVaziaException("login");
		}else if(this.pass.getPassword().length <= 0)
		{
